...
function processJsonWithPhaseCAE(data, phase) {
  ...
  Object.keys(data).forEach((activityKey) => {
    ...
    if (typeof activity === "object" && activity !== null) {
      ...
      const multiplier = activity[phaseKey] !== undefined
        ? activity[phaseKey]
        : (activity[phaseKeyLower] !== undefined ? activity[phaseKeyLower] : 1);

      const loops = activity["noOfLoops"] !== undefined ? activity["noOfLoops"] : 1

      // Multiply the core hour components
      if ("TDL" in activity) activity.TDL = (activity.TDL || 0) * multiplier * loops;
      if ("COO" in activity) activity.COO = (activity.COO || 0) * multiplier * loops;
      if ("DE" in activity) activity.DE = (activity.DE || 0) * multiplier * loops;

      // Update the standard total based on the new multiplied values
      activity.standard = (activity.TDL || 0) + (activity.COO || 0) + (activity.DE || 0);
...
function updateIndustrializationValuesFromQuotation(
  ...
  const quot3DStandard = quotationData?.Industrialization?.activities?.["Industrialization | 3D modifications"]?.standard || 0;
  const quotFeasibilityStandard = quotationData?.Industrialization?.activities?.["Industrialization | Feasibility"]?.standard || 0;
  ...
  for (let key in mappings) {
    ...
    if (mappingKey === "_3DModification") {
      ...
      detailedJson[key].DE = quotationData?.Industrialization?.activities?.["Industrialization | 3D modifications"]?.DE;
      detailedJson[key].COO = quotationData?.Industrialization?.activities?.["Industrialization | 3D modifications"]?.COO;
      detailedJson[key].TDL = quotationData?.Industrialization?.activities?.["Industrialization | 3D modifications"]?.TDL;
  ...
  console.log("Updated Industrialization values based on quotation:", detailedJson);
  return detailedJson;
}
...
resolver.define("getConfigData", async ({ payload }) => {
  ...
  if (key == "Activity") {
    ...
    const hasQuotationReference = issueDetail.fields["customfield_12059"]?.value ?? "No";
    ...
    if ((isCAE || isPS) && (phase === "Offer" || phase === "Industrialization")) {
      console.warn(`Phase '${phase}' is not applicable for ${isCAE ? "CAE" : "PS"}. Only Proto and Serie are allowed.`);
      updatedActivities = {};
    }
    ...
    // 5. Handle CAD/PS Phases (Proto, Serie, or CAD Industrialization)
      ...
      if (phase === "Proto" || phase === "Serie") {
        ...
        if (data[key] && data[key].activities) {
          ...
          if (quotation && quotationVersion && hasQuotationReference === "Yes") {
            _2DDrawing = data["2D Drawing - Quotation"]?.activities[projectType]?.[key];
          ...
          if (quotation && quotationVersion && hasQuotationReference === "Yes") {
            _dataManagementTime = data["Data Management - Quotation"][projectType]?.["customers"]?.[key]?.[customer] || data["Data Management - Quotation"][projectType]?.["customers"]?.[key]?.["Standard"];
            _dataManagementPercentages = data["Data Management - Quotation"]?.percentages;
          ...
          console.log("2D% --> ", _2DDrawingPercentages, "\nDM% --> ", _2DDrawingPercentages, "\nPhases : ", data[key]["Phases"])
      ...
      // Industrialization logic (Inherently CAD-only due to top-level guard clause)
        ...
        if (quotation && quotationVersion && hasQuotationReference === "Yes") { // when linked quotation, use new logic
          ...
          console.log("Check Industrialization Feasibility in quotation data:", Object.hasOwn(data[key]["Industrialization"].activities, "Industrialization | Feasibility"));
          if (Object.hasOwn(data[key]["Industrialization"].activities, "Industrialization | Feasibility")) {
            updatedActivities["Industrialization | Feasibility"] = data[key]["Industrialization"].activities["Industrialization | Feasibility"];
          }

        } else { // when no linked quotation, fallback to old logic (based on Proto/Serie sums)
...
resolver.define("saveQuotationData", async ({ payload }) => {
  ...
  const updateProductPartsResponse = await updateProductParts(issue, existingProductParts, newProductParts);
  ...
  if (workOrderData) {
    ...
    console.log("Setting Work Order data in storage with key: ", `Quot_WO_${product}_${customer}_${issue}_CAD_${quotVersion}`);
  }
...
resolver.define("saveQuotationDataCae", async ({ payload }) => {
  let { issue, data, workOrderData, selectedProductParts, currentVersion, quotVerTotalHours, quotVerTotalCost } =
    payload;
