...
function calculate3DActivity(activityName, activityDetails, globalFlags) {
  ...
  for (const [productName, product] of Object.entries(activityDetails)) {
    ...
    if (hasSubActivity && product.componentSelected) {
       ...
       */
      const protoLoop = product.Proto || 0; // Default to 0 to avoid accidental calculation
      const serieLoop = product.Serie || 0;
  ...
  return {
    // proto only
    protoTDL: protoTDL.toFixed(2),
    protoCOO: protoCOO.toFixed(2),
    protoDE: protoDE.toFixed(2),
    protoTotal: (protoTDL + protoCOO + protoDE).toFixed(2),

    // serie only
    serieTDL: serieTDL.toFixed(2),
    serieCOO: serieCOO.toFixed(2),
    serieDE: serieDE.toFixed(2),
    serieTotal: (serieTDL + serieCOO + serieDE).toFixed(2),

    // combined total
    TDL: totalTDL.toFixed(2),
    COO: totalCOO.toFixed(2),
    DE: totalDE.toFixed(2),
    Total: (totalTDL + totalCOO + totalDE).toFixed(2),
  };
}
