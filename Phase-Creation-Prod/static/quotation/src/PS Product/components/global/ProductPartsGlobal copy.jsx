...
export default function ProductPartsGlobal({ data, applyGlobalChange }) {
  ...
  // /* Initialize UI state */
  // useEffect(() => {
  //   if (Object.keys(productUI).length === 0) {
  //     const initial = {};
  ...
  //     products.forEach((product) => {
  //       // ✅ READ FROM activities
  //       const saved = Object.values(data?.activities || {}).find(
  //         (a) => a?.[product]
  //       )?.[product];

  //       next[product] = {
  //         componentSelected: saved?.componentSelected ?? false,
  //         noOfComponent: saved?.noOfComponent ?? 1,
  //       };
  //     });
