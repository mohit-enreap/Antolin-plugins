...
export function useActivityLogic() {
  ...
  const [globalParams, setGlobalParams] = useState({
    ...
    COO: "NO",
    DE: "NO",
    // "2D": "NO",  // Commented also commneted in ActivityLogic Hook
  ...
  const sopEndDate = useMemo(() => {
    ...
    const totalWeeks =
      (phaseFlags.phase_0 ? phase0Weeks : 0) +
      (phaseFlags.phase_1 ? phase1Weeks : 0) +
      (phaseFlags.phase_2 ? phase2Weeks : 0) +
      (phaseFlags.phase_3_4 ? phase34Weeks : 0);
    // alert("totalWeeks Are: " + totalWeeks);

    if (totalWeeks === 0) return "";

    const start = new Date(sopParams.startDate + "T00:00:00");
    if (isNaN(start)) return "";

    const sop = new Date(start);
    sop.setDate(start.getDate() + totalWeeks * 7);
    // alert("Days Are: " + totalWeeks * 7);

    return sop.toISOString().split("T")[0];
  }, [
    sopParams.startDate,
    phaseFlags,
    phase0Weeks,
