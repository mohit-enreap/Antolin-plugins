...
export function useActivityLogic() {
  ...
  // 1st and 2nd step mould shortcut activities initialization
  const groupedIncomingData = {
    ...
    //   "Thickness map by point (HDL)": 0,
    // "Thickness map by point (Mould)": 0,
  };
  ...
  useEffect(() => {
    ...
    if (storedData.dmData) {
      setDataManagementData((prevDefaults) => ({
        ...prevDefaults, // Keep the defaults ("TDL", "3D Standard", etc.)
        ...storedData.dmData, // Overwrite with data from storage
      }));
    }

    if (storedData.Initial) {
      setActivityValues((prevDefaults) => ({
        ...prevDefaults, // Keep the defaults (grouped structure)
        ...storedData.Initial, // Overwrite with data from storage
      }));
    }
  ...
  useEffect(() => {
    if (!globalParams.Customer) return;

    console.log(
      "USE Effect CAD activity hook --> \n",
      jsonData,
      "\nStringify-->\n",
...
*/
        ...
        if (hasSub) {
          for (const subKey in roof.subactivity) {
            ...
            if (isActivityInProduct(productName, subKey.trim())) {
              ...
              // Always: standard × scalingFactor
              if (typeof sub._standardProto === "number") {
                sub.Proto = sub._standardProto * scalingFactors.Proto;
              }

              // 🔢 Recalculate Serie from STANDARD value
              if (typeof sub._standardSerie === "number") {
                sub.Serie = sub._standardSerie * scalingFactors.Serie;
              }
            }
          }
          ...
          if (isActivityInProduct(productName, activityName)) {
            ...
            // 🔢 Always calculate from STANDARD values
            if (typeof roof._standardProto === "number") {
              roof.Proto = roof._standardProto * scalingFactors.Proto;
            }

            if (typeof roof._standardSerie === "number") {
              roof.Serie = roof._standardSerie * scalingFactors.Serie;
            }
          }
        }
      }
  ...
  function getScalingFactor(manualWeeks, standardWeeks) {
    ...
    if (manualWeeks > standardWeeks) {
      return Math.round(manualWeeks / standardWeeks);
    }
  ...
  useEffect(() => {
    setJsonData((prev) =>
      applyScalingFactor(prev, "HEADLINER", scalingFactors),
    );
  }, [scalingFactors]);
  ...
  */
  ...
  //alert("groupedIncomingData:"+ JSON.stringify(activityValues))
  ...
  useEffect(() => {
    ...
    //old
    // const { counter, activityFactors } =
    //   calculateScalingfactorAndCounterforOHSHelper(jsonData);
    //=======new start===
    //    const incomingData = {
    ...
    // const incomingData =  // SHORTCUT_1st2ndStepMould_ACTIVITIES_LISTACTIVITIES_LIST;

    // Calling main logic to handle couter and factor for 1step 2step mould shortcut
    const { counter, activityFactors } =
      calculateScalingfactorAndCounterforOHSHelper(jsonData, activityValues);
    //==== new ends=========

    setCounter(counter);
    setActivityFactors(activityFactors);
    console.log("Counter:", counter);
    console.log(counter + "Activity Factors:", activityFactors);
  }, [activityValues, jsonData]);

  // to  "1 Step Mould" in table in synch with Counter
  useEffect(() => {
    updateActivityValue("2 Steps Mould", "1 Step Mould", counter);
  }, [counter]);
  ...
  const updateActivityValue = (section, name, value) => {
    ...
    setActivityValues((prev) => {
      ...
      // ✅ ONLY apply logic if section has both fields
      if (isStepMouldSection(prevSection)) {
        let oneStep = Number(prevSection["1 Step Mould"]) || counter;
      ...
      //  NORMAL fields (no special logic)
      return {
        ...prev,
        [section]: {
          ...prevSection,
  ...
  // start mould
  // to handle JSON change runti

  // 2️⃣ THEN: use them
  useEffect(() => {
    if (!jsonData || !activityFactors) return;

    const updated = applyMouldShortcutFactors(jsonData, activityFactors);

    setJsonData((prev) => {
      if (JSON.stringify(prev) === JSON.stringify(updated)) {
        return prev; // stop re-render loop
      }
      return updated;
    });
  }, [jsonData, activityFactors]);
  ...
  ====================================================== */
    setAddOnComponentState((prev) => {
      ...
      // - noOfComponent > 0
      updated.componentSelected = componentSelected && noOfComponent > 0;
  ...
  function getDataManagementRows() {
    ...
    for (const activityKey in DATA_MANAGEMENT_DATA_PHASE) {
      ...
      const hoursPerWeek = DATA_MANAGEMENT_DATA_PHASE[activityKey].hours;
...
displaying the contents of a variable `costCentersData` using `JSON.stringify`.  */
  ...
  // this require (dontb ignore)
  const CECO_COST_SHEET_YearANDPhases1 = {
    data: [
      {
        society: "Antolin China Investment",
        ceco: "ACIIF",
