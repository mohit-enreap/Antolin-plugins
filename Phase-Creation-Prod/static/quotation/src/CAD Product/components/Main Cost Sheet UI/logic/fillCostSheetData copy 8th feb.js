...
const updatedRows3 = template.rows3.map((row) => {
  ...
  if (roleKey === "Thermals Activities") {
    ...
    phase1Hours = 24   *  phaseMultiplier.phase1;;
    phase2Hours = 16  * phaseMultiplier.phase2;;
    phase3Hours = 1  * phaseMultiplier.phase3;;
  } else {
    return row;
  }
...
const updatedRows4 = template.rows4.map((row) => {
  ...
  if (roleKey === "P. Safety Standard & Leader act") {
    phase0Hours = Number(sharedRef?.currentPS?.protoTotal ?? 0) *  phaseMultiplier.phase0 ;
    phase1Hours = Number(sharedRef?.currentPS?.protoTotal ?? 0) *  phaseMultiplier.phase1 ;
    phase2Hours = Number(sharedRef?.currentPS?.serieTotal ?? 0) *  phaseMultiplier.phase2 ;
    phase3Hours = Number(sharedRef?.currentPS?.protoTotal ?? 0) *  phaseMultiplier.phase3 ;
  } else {
    return row;
  }
...
const updatedRows5 = template.rows5.map((row) => {
  ...
  if (categoryKey === "Travels") {
    ...
    phase1Hours = MaterialsPhase1.hours;          // ✅ Hard-coded costs . later we will get from UI
...
const cleaned = Object.fromEntries(
  Object.entries(sharedRef.costCenters || {}).map(([k, v]) => [
    k,
    v?.new ?? v
  ])
);