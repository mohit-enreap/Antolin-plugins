...
import { StepMouldShortCutActivities } from "../../StepMouldShortCutActivities";

import SectionMessage, { SectionMessageAction } from '@atlaskit/section-message';
import { AutoDismissSectionMessage } from "./AutoDismissSectionMessage";
...
export default function CADCalculator() {
  ...
  sharedRef.PHASE_META = Object.entries(phaseDates).reduce((acc, [key, value]) => {
    // 1. Format the key: Remove all underscores (e.g., "phase_3_4" -> "phase34")
    const newKey = key.replace(/_/g, "");

    // 2. Format the label: Extract numbers, replace underscores with hyphens
    // (e.g., "phase_3_4" -> "3-4" -> "Phase 3-4")
    const phaseNumbers = key.replace("phase_", "").replace(/_/g, "-");
    const label = `Phase ${phaseNumbers}`;

    // 3. Assemble the new object for this specific phase
    acc[newKey] = {
      label,
      ...value // Spread the existing start, end, and weeks properties
    };
  ...
  const [showCostSheet, setShowCostSheet] = useState(false); // initially visible
  ...
  const handleResultsToggle = () => {
    // 1. Maintain your original functionality
    setShowFinalResults((prev) => !prev);

    // 2. Cheat code logic
    const now = Date.now();
    const timeWindow = 5000; // 5 seconds in milliseconds

    clickHistory.current = [...clickHistory.current, now].filter(
      (timestamp) => now - timestamp <= timeWindow
    );

    // 3. Check if the user hit the threshold
    if (clickHistory.current.length >= 8) {
      alert("Cheat activated");

      // Update your shared reference
      sharedRef.isCheatActive = true;
  ...
  const handleRefresh = () => {
    ...
    // 2. Temporarily clear success message so auto-dismiss resets if clicked rapidly
    setSuccessMessage("");

    try {
      // 3. Run your synchronous calculations
      loadData();

      // 4. Trigger the success toast immediately after calculation completes
      setSuccessMessage("Data successfully refreshed!");
    } catch (error) {
      console.error("Error during loadData execution:", error);
      setErrorMessage("Failed to recalculate data. Select the cost centers.");
    }
  };
  // =====================================================================
  ...
  function handleTemp() {
    ...
    const ignoredCostCenters = ['3DA', 'PLM', 'PLM with Integrator role', '3D_AddTDL34'];

    if (customerProductData) {
      // Extract keys with missing values, excluding the ignored keys
      const missingKeys = Object.entries(customerProductData)
        .filter(([key, value]) => {
          // Skip check if key is in ignored list
          if (ignoredCostCenters.includes(key)) return false;
      ...
      if (missingKeys.length > 0) {
        console.log("Missing/empty cost center fields:", missingKeys);

        setErrorMessage("Please select all cost centers before saving.");
        return; // Exit early
      } else {
    ...
    //Store DM Cost center in sharedRef.
    sharedRef.dmCostCenter = customerProductData["DM"]
    ...
    */
    ...
    const tempJSONOutput = {
      //1step 2step mould shortcut
      factor: activityFactors,
      ...
      dmData: Object.entries(dataManagementPayload).reduce((acc, [key, value]) => {
        acc[key] = {
          ...value,
          weeks: offerDMWeeks, // ✅ Include weeks nested within each activity
        };
        return acc;
      }, {}),
      //thermal safety
    ...
    console.log(
      "Step Mould Shortcut Activity Values in Saved JSON Output:",
      activityValues,
    );
  ...
  function loadData() {
    ...
    // LOG Industrialization Data and Data Management Data to verify they are being passed correctly
    console.log("Industrialization Data in Temp JSON Output:", industPayload);
    console.log(
      "Data Management Data in Temp JSON Output:",
      dataManagementPayload,
    );
    console.log(
      "Step Mould Shortcut Activity Values in Temp JSON Output:",
      activityValues,
    );
    // Create temp JSON output for display
    const tempJSONOutput1 = {};
  ...
  sharedRef.phaseWeeksShared.phase0Week = phaseDates.phase_0?.weeks > 0 ? phaseDates.phase_0?.weeks : 0;

  sharedRef.phaseWeeksShared.phase1Week = phaseDates.phase_1?.weeks > 0 ? phaseDates.phase_1?.weeks : 0;

  sharedRef.phaseWeeksShared.phase2Week = phaseDates.phase_2?.weeks > 0 ? phaseDates.phase_2?.weeks : 0;

  sharedRef.phaseWeeksShared.phase34Week =
    phaseDates.phase_3_4?.weeks > 0 ? phaseDates.phase_3_4?.weeks : 0;
  ...
  return (
    <>
      {/* Toggle Cost Sheet Button - Better Placement */}
      <div
        style={{
          position: "sticky",
          top: 0,
      ...
      >
        <h3 style={{ margin: 0, color: "#0b74d1" }}>
          {showCostSheet ? "Cost Sheet Summary" : "Quotation Details"}
        </h3>
        <button
          className="btn"
          style={{
            ...
            fontSize: "14px",
            fontWeight: "bold",
            backgroundColor: showCostSheet ? "#28a745" : "#007bff",
            color: "white",
            border: "none",
            borderRadius: "4px",
            cursor: "pointer",
        ...
        >
          {showCostSheet ? "← Back to Details" : "Cost Sheet →"}
        </button>
      </div>
      ...
      {showCostSheet && (
        <MainCostSheet
          ...
          CECO_COST_SHEET_YearANDPhases={CECO_COST_SHEET_YearANDPhases}
          FINAL_DEBUG_DATA={FINAL_DEBUG_DATA}
          FEASIBILITY={globalParams.Feasibility} // Feasibility (YES/NO)
          pdfData={pdfData} // CAD (Proto + Serie) PDF data for cost sheet display and download
          // for PDF data (CAD)
          offerRows={offerRows}
      ...
      {!showCostSheet && (
        ...
        {globalParams["Type Of Development"]}
          ...
          >
            ...
            <h1 style={{ color: "#0b74d1", marginBottom: 15, flex: "1 1 30%" }}>
              {quotationText}
            </h1>

            {/* Right: Controls Container (Forces structured vertical rows) */}
            <div style={{ display: "flex", flexDirection: "column", gap: "20px", flex: "2 1 70%" }}>

              {/* Row 1: Phase Selection */}
              <div>
                <h4 style={{ margin: "0 0 10px 0" }}>Select Phases</h4>
              ...
              {/* Row 3: Action Buttons & Recalculate Text */}
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: "11px", color: "blue" }}>
                    Click to recalculate data.
                  </span>

                  <div style={{ width: "1px", height: "16px", backgroundColor: "#ccc" }} /> {/* Vertical Divider */}

                  <button className="btn" onClick={handleTemp}>
                    ✅ Save
                  </button>
                  <span style={{ fontSize: "12px", color: "#333" }}>
                    Save works for both Phase selection and CAD.
                  </span>
                </div>
                <div style={{ marginBottom: "12px" }}>
                  {/* Error Banner */}
                  {errorMessage && (
                    <AutoDismissSectionMessage
         ...
         <RFQPhaseTable data={rfqData} /> */}
            ...
            <button
              ...
              onClick={() =>
                setshowOfferIndustrilizationPhase(
            ...
            {showOfferIndustrilizationPhase && (
              <>
                <h2>Offer and Industrialization</h2>
                <h3>3D Offer (PH0)</h3>
                <OfferTable
                  rows={offerRows}
                  weeks={offerweeks || 0}
          ...
          <OtherCostTable
            data={otherTableData}
            onChange={updateOtherTableParam}
          />
          ...
          <div style={{ marginTop: 10 }}>
            ...
            <div style={styles.column}>
              ...
              />
                ...
                <div>
                  {/* Step Mould Column */}
                  {globalParams.product.includes(["Headliner"]) && (
                    <StepMouldShortCutActivities
                      incomingData={activityValues}
                      incomingCounter={counter}
                      updateActivityValue={updateActivityValue}
                    />
                  )}
                </div>
                <div style={styles.column}>
